...
async function getProvider(denops: Denops): Promise<Provider> {
  ...
  try {
    ...
    const preferredProvider = await vars.g.get(denops, "gitxab_provider", "auto") as string;
    
    if (preferredProvider === "auto") {
      // Auto-detect provider
      const detectedProvider = await detectCurrentProvider();
      await denops.cmd(`echomsg 'GitXab: Detected provider: ${detectedProvider}'`);
      providerInstance = await createProvider();
    } else {
      // Use specified provider
      await denops.cmd(`echomsg 'GitXab: Using configured provider: ${preferredProvider}'`);
      providerInstance = await createProvider({
    ...
    await denops.cmd(`echohl ErrorMsg | echomsg 'GitXab: Failed to initialize provider: ${err.message}' | echohl None`);
    throw error;
  }
}
  ...
  denops.dispatcher = {
      ...
      try {
        ...
        if (preferredProvider === "github") {
          ...
          projects = repositories.map(repo => ({
            id: typeof repo.id === 'number' ? repo.id : parseInt(repo.id, 10) || 0,
            name: repo.name,
            path: repo.fullName,
    ...
    async setProvider(providerType: unknown): Promise<void> {
      try {
        if (typeof providerType !== "string") {
          await denops.cmd('echohl ErrorMsg | echo "Usage: :GitXabSetProvider github|gitlab" | echohl None');
          return;
        }
        
        const provider = providerType.toLowerCase();
        if (provider !== "github" && provider !== "gitlab") {
          await denops.cmd('echohl ErrorMsg | echo "Invalid provider. Use: github or gitlab" | echohl None');
          return;
        }
        ...
        await denops.cmd(`echomsg 'GitXab: Provider set to ${provider}. Re-run commands to use new provider.'`);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        await denops.cmd(`echohl ErrorMsg | echo "Failed to set provider: ${message}" | echohl None`);
      }
    },
    ...
    async showProvider(): Promise<void> {
      try {
        ...
        await denops.cmd(`echomsg 'GitXab: Current provider is ${providerType} (${baseUrl})'`);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        await denops.cmd(`echohl ErrorMsg | echo "Failed to get provider: ${message}" | echohl None`);
      }
    },
    ...
    async listMergeRequests(projectId: unknown): Promise<unknown> {
      try {
        ...
        if (mrs.length === 0) {
          await denops.cmd('echohl WarningMsg | echo "No merge requests found" | echohl None');
          return [];
        }
        ...
        for (const mr of mrs) {
          ...
          const labels = mr.labels && mr.labels.length > 0 ? ` [${mr.labels.join(", ")}]` : "";
        ...
        lines.push("", "Keys: <Enter>=View  n=Create MR  r=Refresh  q=Close  ?=Help");
        ...
        const message = error instanceof Error ? error.message : String(error);
        await denops.call("nvim_err_writeln", `GitXab: Failed to list merge requests: ${message}`);
        return null;
      }
    },
    ...
    async openMRFromList(bufnr: unknown): Promise<void> {
      try {
        ...
        if (!mr) {
          await denops.call("nvim_err_writeln", `GitXab: MR !${mrIid} not found in list`);
          return;
        }
        ...
        const message = error instanceof Error ? error.message : String(error);
        await denops.call("nvim_err_writeln", `GitXab: Failed to open MR: ${message}`);
      }
    },
    ...
    async viewMergeRequest(projectId: unknown, mrIid: unknown): Promise<void> {
      try {
        ...
        if (mr.assignees && mr.assignees.length > 0) {
          lines.push(`Assignees: ${mr.assignees.map((a: { username: string }) => `@${a.username}`).join(", ")}`);
        }
        ...
        const message = error instanceof Error ? error.message : String(error);
        await denops.call("nvim_err_writeln", `GitXab: Failed to view merge request: ${message}`);
      }
    },
    ...
    async commentOnMR(projectId: unknown, mrIid: unknown): Promise<void> {
      try {
        ...
        await denops.cmd(`autocmd! * ${escapedPath}`);
        await denops.cmd(`autocmd BufWritePost ${escapedPath} call denops#request('${denops.name}', 'onMRCommentBufferSave', [${bufnr}])`);
        await denops.cmd(`autocmd BufUnload ${escapedPath} call denops#request('${denops.name}', 'cleanupCommentEdit', ['${tmpFile}'])`);
        await denops.cmd(`augroup END`);
    ...
    async replyToMRComment(projectId: unknown, mrIid: unknown): Promise<void> {
      try {
        ...
        // Get stored discussions from buffer
        const discussions = await fn.getbufvar(denops, "%", "gitxab_mr_discussions") as Discussion[] | string;
        if (!discussions || typeof discussions === "string" || discussions.length === 0) {
          await denops.cmd('echohl WarningMsg | echo "No discussions found" | echohl None');
          return;
        }
        ...
        const tmpFile = `${tmpDir}/.gitxab_${projectId}_mr${mrIid}_reply_${discussionNum}.md`;
    ...
    async onMRCommentBufferSave(bufnr: unknown): Promise<void> {
      try {
        ...
        const projectId = await vars.b.get(denops, "gitxab_project_id") as number;
        const mrIid = await vars.b.get(denops, "gitxab_mr_iid") as number;
        const action = await vars.b.get(denops, "gitxab_action") as string;
        const discussionId = await vars.b.get(denops, "gitxab_discussion_id") as string | undefined;
        ...
        if (!body) {
          await denops.cmd('echohl WarningMsg | echo "Comment is empty, not posted" | echohl None');
          return;
        }
        ...
        if (action === "mr_comment") {
          ...
          await denops.cmd(`echohl MoreMsg | echo "✓ Comment posted to MR !${mrIid}" | echohl None`);
        } else if (action === "mr_reply" && discussionId) {
          await denops.cmd('echo "Posting reply to discussion..."');
          const discussionNum = await vars.b.get(denops, "gitxab_discussion_num") as number;
    ...
    async viewMRDiffs(projectId: unknown, mrIid: unknown): Promise<void> {
      try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        await denops.call("nvim_err_writeln", `GitXab: Failed to view diffs: ${message}`);
      }
    },
      ...
      try {
        ...
        try {
          ...
          if (Array.isArray(branchData)) {
            branches = branchData;
            // Find default branch
            const defBranch = branches.find((b: { default?: boolean }) => b.default);
            if (defBranch) {
        ...
        if (branches.length > 0) {
          ...
          const branchList = branches.slice(0, 20).map((b: { name: string; default?: boolean }) => 
        ...
        await denops.cmd(`autocmd! * ${escapedPath}`);
        await denops.cmd(`autocmd BufWritePost ${escapedPath} call denops#request('${denops.name}', 'onCreateMRBufferSave', [${bufnr}])`);
        await denops.cmd(`autocmd BufUnload ${escapedPath} call denops#request('${denops.name}', 'cleanupCommentEdit', ['${tmpFile}'])`);
        await denops.cmd(`augroup END`);
        
        await denops.cmd('echo "Fill in the MR form, :w to create, :q to cancel"');
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        await denops.call("nvim_err_writeln", `GitXab: Failed to open MR creation form: ${message}`);
      }
    },
    ...
    async onCreateMRBufferSave(bufnrParam: unknown): Promise<void> {
      try {
        ...
        if (!formData.source_branch || formData.source_branch === "") {
          await denops.cmd('echohl ErrorMsg | echo "Error: source_branch is required" | echohl None');
          return;
        }
        
        if (!formData.target_branch || formData.target_branch === "") {
          await denops.cmd('echohl ErrorMsg | echo "Error: target_branch is required" | echohl None');
          return;
        }
        
        if (!formData.title || formData.title === "") {
          await denops.cmd('echohl ErrorMsg | echo "Error: title is required" | echohl None');
          return;
        }
        ...
        // Show success message
        await denops.cmd(`echohl MoreMsg | echo "✓ Merge Request !${mr.iid} created successfully" | echohl None`);
        
        // Refresh MR list if we're in an MR buffer
        const prevBufnr = await fn.bufnr(denops, "#");
        const filetype = await fn.getbufvar(denops, prevBufnr, "&filetype") as string;
        if (filetype === "gitxab-mrs") {
          await denops.dispatcher.listMergeRequests(projectId);
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        await denops.call("nvim_err_writeln", `GitXab: Failed to create MR: ${message}`);
      }
    },
  };
