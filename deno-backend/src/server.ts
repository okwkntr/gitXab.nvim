...
const IPC_SOCKET_PATH = Deno.env.get("GITXAB_SOCKET_PATH") || "/tmp/gitxab.sock";
...
function httpHandler(req: Request): Response {
  ...
  if (url.pathname === "/projects") {
    ...
    const results = MOCK_PROJECTS.filter(p => p.name.includes(q) || p.description.includes(q));
    ...
    while (true) {
      ...
      while ((idx = partial.indexOf("\n")) !== -1) {
        ...
        try {
          ...
          await writeNdjson(conn, { id: null, error: { code: 1000, message: "invalid json" } });
          continue;
        }
        // handle methods
        if (msg.method === "list_projects") {
          ...
          await writeNdjson(conn, { id: msg.id || null, error: { code: 1001, message: "method not found" } });
        }
      }
    }
  } catch (e) {
...
try {
  // remove stale socket
  try { await Deno.remove(IPC_SOCKET_PATH); } catch {}
  ...
  console.log("UDS not available, will continue to TCP fallback: ", e.message || e);
}
